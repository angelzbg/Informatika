...
public class Producer extends Thread {      // Producer thread
    ...
    public void run() {                     
        ...
        while(true) {                       
            ...
            try{
                if(count == N) wait();      // If the buffer is full, go to 
                                            // sleep
            ...
            count++;                        // Increment count of items in 
                                            // buffer
            if(count == 1) c.notify();      // Was the buffer empty?
    }
    ...
}
...
public class Consumer extends Thread {      // Consumer thread
    ...
    public void run() {
        ...
        while(true) {
            try{
                if(count == 0) wait();  // If the buffer is empty, go to sleep
            } catch(InterruptedException) {};
            
            item = remove_item();           // Get the item from the buffer
            count--;                        // Decrement count of items in 
                                            // buffer
            if(count == N-1) p.notify();    // Was the buffer full?
